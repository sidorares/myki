...
function renderCurrentLine(self, stringToWrite, showSuggestions) {
  ...
  function afterSuggestion(suggestion) {
    ...
    self.colorize(stringToWrite, (err, colorized) => {
      if (suggestion && suggestion.indexOf(stringToWrite) === 0) {
        self._ansiCursor.horizontalAbsolute(promptLength + 1).eraseLine().write(colorized + chalk.grey(suggestion.substring(stringToWrite.length)));
        self._ansiCursor.horizontalAbsolute(nX + 1);
      } else {
        self._ansiCursor.horizontalAbsolute(promptLength + 1).eraseLine().write(colorized);
